...
# Installs JUST the UPS info transmission daemon
#
# Remove service with:
# systemctl disable charlotte-ups-send.service
# systemctl stop charlotte-ups-send.service
# rm /lib/systemd/system/charlotte-ups-send.service
...
SERVICE="systemd"
FILENAME="charlotte-ups-send.service"
...
[Unit]
Description=Manage Charlotte UPS info transmission daemon

[Service]
User=pi
Type=oneshot
ExecStart=/usr/bin/env bash -c 'cd $DIR/..; \$PWD/bin/ups_send_daemon.py start'
ExecStop=/usr/bin/env bash -c 'cd $DIR/..; \$PWD/bin/ups_send_daemon.py stop'
RemainAfterExit=yes
...
systemctl enable charlotte-ups-send.service
if [ "$?" -ne "0" ];
then
  echo ""
  echo 1>&2 "Failed to enable Charlotte UPS info transmission service in Systemd"
  exit 1
else
  echo "Done"
fi
...
then
  echo ""
  echo 1>&2 "Failed to start Charlotte UPS info transmission service via Systemd"
  exit 1
else
  echo "Started"
fi
